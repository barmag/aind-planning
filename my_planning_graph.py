...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            ...
            #check if all prerequisitie literals hold
            if action_node.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(action_node)

                for state in action_node.prenodes:
                    state.children.add(action_node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        intersection = len(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)) + len(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos))
        intersection += len(set(node_a2.action.effect_add) & set(node_a1.action.precond_neg)) + len(set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos))
        return intersection > 0
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_parents in node_a1.parents:
            for a2_parents in node_a2.parents:
                if a1_parents.is_mutex(a2_parents):
                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        is_negated = node_s1.is_pos != node_s2.is_pos and node_s1.symbol == node_s2.symbol
        return is_negated
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            node = PgNode_s(g, True)
            for i, nodes in enumerate(self.s_levels):
                if node in nodes:
                    level_sum += i
                    break
        return level_sum
